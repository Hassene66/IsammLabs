...
const AdvancedChecklistButton = ({
  placeholder = 'Liste',
  list = [],
  name,
  style = {},
  getSelectedIdsAndItems = () => {},
  setReload = () => {},
  selectedSoftware = [],
  setLoading = () => {},
}) => {
  ...
  const onSelectItem = ({ids, items}) => {
    getSelectedIdsAndItems(name, ids, items);
  };
  let filteredData = useMemo(
    () =>
      filter(list, {
        keywords: `name:${searchTerm}`,
        caseSensitive: false,
      }).sort(function (a, b) {
        if (a.name < b.name) {
          return -1;
  ...
  const addNewSoftware = () => {
    setLoading(true);
    softwareService
      .addSoftwareApi({name: searchTerm.toLowerCase()})
      .then(() => {
        setSearchTerm('');
      ...
      <View>
        <RBSheet
          ...
          <View style={styles.list}>
            ...
            />
            <CheckboxList
              selectedListItems={selectedSoftware[name] || []}
...
export default AdvancedChecklistButton;
